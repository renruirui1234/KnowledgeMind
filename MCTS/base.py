...
import numpy as np
import os
# from Agent.MCTS.Log_Agent import *
# from Agent.MCTS.Metric_Agent import *
from Agent.Trace_Agent import *
import pandas as pd

from MCTS.Agent.Log_Agent import Log_analysis


class treeNode(object):
    def __init__(self, pcd,analysis_path, parent=None, depth=0,llm_flag=0):
        self.pcd = pcd  # str
        self.y = ''  # str
        ...
        self.analysis = self.analysis_result()
        self.llm_evaluation=None


    def ReAct_analysis(self):
        #could design the react prompt and analyze abnormal metric
        return ""


    def analysis_result(self):
        if self.llm_flag:#leverage LLM to call function and analyze result
            self.ReAct_analysis()
        else:#default analyze trace、log and metric
            metric_result=self.get_metric_result()
            log_result_1,log_result_2=self.get_log_result()
            result=f'''
            Metric分析结果:
            {metric_result}
            以上Metric存在异常
    ...
    def get_metric_df(self,service):
        metric_path = os.path.join("./output_anomaly", self.analysis_path + "_anomalies.csv")
        df=pd.read_csv(metric_path)
        #check alarm and SPOT for further
        return df[df["cmdb_id"].str.contains(service, na=False)]

    def get_log_df(self,service):
        log_path_1 = os.path.join("./output_anomaly_log", self.analysis_path, self.analysis_path + "_log_1.csv")
        df_1 = pd.read_csv(log_path_1)
        log_path_2 = os.path.join("./output_anomaly_log", self.analysis_path, self.analysis_path + "_log_2.csv")
        df_2 = pd.read_csv(log_path_2)
        return df_1[df_1["cmdb_id"].str.contains(service, na=False)], df_2[
            df_2["cmdb_id"].str.contains(service, na=False)]
    ...
    def get_metric_result(self):
        ...
        df=pd.read_csv(metric_path)
        #check alarm and SPOT for further
        return df[df["cmdb_id"].str.contains(self.pcd, na=False)].to_string(index=False)
    ...
    def get_log_result(self):
        ...
        log_path_2 = os.path.join("./output_anomaly_log",self.analysis_path, self.analysis_path+"_log_2.csv")
        df_2=pd.read_csv(log_path_2)
        return df_1[df_1["cmdb_id"].str.contains(self.pcd, na=False)].to_string(index=False),df_2[df_2["cmdb_id"].str.contains(self.pcd, na=False)].to_string(index=False)
    ...
    def append_children(self, node):
        ...
        self.children.update({node.pcd: node})
        return self
