...
def detect_anomalies(data,file_name, column_name, mean, std):
    anomalies = data[abs(data[column_name] - mean) > 3*abs(mean)]
    if file_name == "kpi_container_cpu_cfs_throttled_periods.csv":
        anomalies=anomalies[anomalies[column_name]>5]
    return anomalies


# 使用三sigma法进行异常检测
def detect_anomalies_3sigma(data, column_name, mean, std):
    ...
    for folder_name in os.listdir(fault_data_root):
        ...
        if os.path.isdir(folder_path):
            ...
            for file in glob.glob(os.path.join(folder_path, '*.csv')):
                # if file not in ["kpi_container_fs_writes_MB.csv","kpi_container_cpu_usage_seconds.csv","kpi_container_memory_cache.csv","kpi_container_memory_failures.csv"]:
                #     continue
                    ...
                    if 'service' in fault_data.columns and 'count' in fault_data.columns:
                        ...
                        for k in ["rr","sr","mrt","count"]:
                            ...
                            for (cmdb_id, kpi_name), cmdb_kpi_data in train_data.groupby(['cmdb_id', 'kpi_name']):
                                ...
                                anomalies_fault = detect_anomalies(test_kpi_data,file, 'value', mean, std)
                                # anomalies_fault = detect_anomalies_3sigma(fault_data, 'value', mean, std)
